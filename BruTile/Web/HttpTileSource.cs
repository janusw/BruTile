...
public class HttpTileSource(
    ...
    Attribution? attribution = null) : IHttpTileSource, IUrlBuilder
{
    ...
    public HttpTileSource(
        ...
        : this(tileSchema, new BasicUrlBuilder(urlFormatter, serverNodes, apiKey), httpClient, name ?? "", persistentCache, attribution)
    { }
    ...
    /// </summary>
    public virtual async Task<byte[]?> GetTileAsync(TileInfo tileInfo, CancellationToken? cancellationToken = null)
    {
        ...
        bytes = await GetTileAsync(tileInfo, cancellationToken ?? CancellationToken.None);
    ...
    private async Task<byte[]?> GetTileAsync(TileInfo tileInfo, CancellationToken cancellationToken)
    {
